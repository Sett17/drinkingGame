const options = {
    ...
    rumble: {
        ...
        changed: (arg) => {
            if (arg === '0') {
                options.rumble.rumble = true
                rumbler.doRumble()
            } else if (arg === '1') {
                options.rumble.rumble = false
            }
            sess.set('rumble', options.rumble.rumble)
            options.rumble.update()
        },
        get: () => {
            if (options.rumble.rumble === undefined) {
                let sessVal = sess.get('rumble')
                if (isNaN(sessVal)) {
                    options.rumble.rumble = true
                } else {
                    options.rumble.rumble = sessVal
                }
            }
            return options.rumble.rumble
        },
        update: () => {
            let z = options.rumble.get()
            switch (z) {
                case true:
